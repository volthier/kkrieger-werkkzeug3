...
/**
 * Fixed-point math utilities ported from Werkkzeug3's integer math helpers.
 *
 * <p>All values are expressed in 1:15:16 fixed-point units unless documented otherwise.</p>
 */
public final class FixedMath {
    public static final int FIX_ONE = 0x4000;
    public static final int FIX_HALF = 0x2000;
    public static final int FIX_SHIFT = 14;

    private static final int SIN_TAB_SHIFT = 5;
    private static final int SIN_TAB_MASK = (1 << SIN_TAB_SHIFT) - 1;
    private static final int SIN_TAB_SIZE = (FIX_ONE / 4) >> SIN_TAB_SHIFT;
    ...
    private static final int[] ISQRT_TABLE = {
    ...
    public static int iMul(int a, int b) {
        return (int) (((long) a * b) >> FIX_SHIFT);
    }

    public static int iSin(int angle) {
        int adjusted = angle & 0x3fff;
        if (adjusted < 0x2000) {
            if (adjusted >= 0x1000) {
                adjusted = 0x2000 - adjusted;
            }
            int i = adjusted >> SIN_TAB_SHIFT;
            int f = adjusted & SIN_TAB_MASK;
            return (SIN_TABLE[i] + (((SIN_TABLE[i + 1] - SIN_TABLE[i]) * f) >> SIN_TAB_SHIFT)) / 2;
        }
        adjusted -= 0x2000;
        if (adjusted >= 0x1000) {
            adjusted = 0x2000 - adjusted;
    ...
    public static int iCos(int angle) {
        return iSin(angle + FIX_ONE / 4);
    }
    ...
    public static int intSqrt15D(int value) {
        ...
        int i = (int) (x >>> 24);
        int f = (int) (x & 0x00ffffffL);

        long estimate = SQRT_TABLE[i] + mul24(SQRT_TABLE[i + 1] - SQRT_TABLE[i], f) + 0x10000L;
        if ((leading & 1) == 1) {
            estimate >>= 1;
        } else {
            estimate = mul24((int) (estimate + 1), 11_863_283);
        }
        return (int) (estimate >> (leading / 2));
    }
    ...
    public static int iSqrt15D(int value) {
        ...
        long estimate = ISQRT_TABLE[i] + mul24(ISQRT_TABLE[i + 1] - ISQRT_TABLE[i], f);
        if ((leading & 1) == 0) {
            estimate = mul24((int) (estimate + 1), 11_863_283);
        }
        return (int) (estimate >> (15 - leading / 2));
    }

    private static int mul24(int a, int b) {
        return (int) (((long) a * b) >> 24);
    }
}
